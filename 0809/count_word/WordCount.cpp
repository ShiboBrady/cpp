...
void WordCount::writefile(const string &filename)
{
    ofstream out;
...
ifstream &WordCount::openfileRead(ifstream &os, const string &filename)
{
    os.close();
    os.clear();
...
ofstream &WordCount::openfileWrite(ofstream &os, const string &filename)
{
    os.close();
    os.clear();
    os.open(filename.c_str());
