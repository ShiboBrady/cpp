...
string Search::query(const string &word)
{
    ...
    for(it = curSearch.begin(); it != curSearch.end(); ++ it){
        //根据长度差是否大于3来排除一部分词
        ...
        it->second.isSet_ = true;
    }
    string result = search(curSearch);
...
string Search::search(const map<string, Word> &curSearch)
{
    Word maxWord;
    maxWord.word_= "";
    maxWord.med_ = 5;
    ...
    for(it = curSearch.begin(); it != curSearch.end(); ++it){
        if(it->second.isSet_ && maxWord < it->second)
            maxWord = it->second;
    }
